...
export default function MessagesPage() {
  return (
    <div className="min-h-screen bg-[#f9faf2]">
      <Navbar />
      <div className="max-w-[1280px] mx-auto px-6 py-8" style={{ height: "calc(100vh - 73px)" }}>
        <div className="h-full bg-white border border-[#e2e3db] rounded-2xl shadow-[0px_4px_20px_0px_rgba(27,67,50,0.06)] overflow-hidden flex">
          {/* Sidebar */}
          <div className="w-80 border-r border-[#e2e3db] flex flex-col shrink-0">
            ...
            <div className="overflow-y-auto flex-1">
              {conversations.map((conv, i) => (
                <div key={conv.id} className={`p-4 border-b border-[#e2e3db] flex items-start gap-3 cursor-pointer hover:bg-[#f9faf2] transition-colors ${i === 0 ? "bg-[#f3f4ec]" : ""}`}>
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="p-5 border-b border-[#e2e3db] flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-[#1b4332] flex items-center justify-center text-white text-sm font-semibold">J</div>
              <div>
                <p className="font-semibold text-[#1a1c18] text-base">Jordan M.</p>
                <p className="text-[#717973] text-xs">Re: Mountain Bike · $50</p>
              </div>
